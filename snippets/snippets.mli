...
val path_and_filename_of_fullpath : string -> string * string
(** Given the fullpath of a file (such as [/usr/lib/sundials.so]) this
    will return a tuple with the path and the filename
    (i.e. [("/usr/lib","sundials.so")] for this example. *)
...
val string_starts_with: string array -> string -> int option

val preparse_cmdline: string array -> string ->
  (string, string array) Hashtbl.t -> string array -> unit;;
...
val read_double_from_string: string -> bool -> float
...
val memory_footprint: 'a -> (float * float * float)
...
val pq_make : ('a -> 'b) -> ('c, 'b, 'a) pqueue
val pq_is_empty : ('a, 'b, 'c) pqueue -> bool
val pq_nr_entries : ('a, 'b, 'c) pqueue -> int
val pq_push : ('a, 'b, 'c) pqueue -> 'a -> 'c -> unit
val pq_pop : ('a, 'b, 'c) pqueue -> 'a * 'c
val pq_peek : ('a, 'b, 'c) pqueue -> 'a * 'c
val pq_pop_by_name : ('a, 'b, 'c) pqueue -> 'b -> 'a * 'c
val pq_peek_by_name : ('a, 'b, 'c) pqueue -> 'b -> 'a * 'c
val pq_reschedule_by_name : ('a, 'b, 'c) pqueue -> 'b -> 'a -> unit
val pq_iter : ('a, 'b, 'c) pqueue -> ('a -> 'c -> unit) -> unit
val pq_check : (float, 'a, 'b) pqueue -> unit

(* --- 2d adaptive gaussian integradion --- *)

val integrate2d :
  ?max_evaluations:int ->
  ?tolerance:float ->
  points:float array array ->
  simplices:int array array ->
  (float array -> float) -> float * float * int * int

(* --- oommf related --- *)

val interpolate_on_grid :
  float array ->
  float array -> int array -> (int array -> float) -> float array -> float

val oommf_data :
  string ->
  float array * float array * int array * float array array array array

val interpolated_oommf_data : string -> float array -> float array
