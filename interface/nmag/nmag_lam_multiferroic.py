...
def nmag_lam(timestepper,
    ...
    mwe_Electric_ext=ocaml.mwe_sibling(mwe_H_demag,"Electric_ext","Electric_ext/H_demag",[("H_demag","Electric_ext")])
    ...
    if do_stt: mwes_for_dmdt += ["dm_dcurrent"]
...
"""
    ...
    template_op_grad_m = "<grad_m_$MAT$(i, j)||d/dxj m_$MAT$(i)>"
...
"""
    ...
    if need_grad_m:
        str_grad_m = vivify_from_terms("",
                                       #[("$STT_PREFACTOR$", template_op_grad_m)], # XXX T.F. Did I break something?
                                       [("", template_op_grad_m)],
                                       concat=" + ",
    ...
    if need_grad_m:
        ...
        lam_programs["set_grad_m"] = \
          nlam.lam_program("set_grad_m",
                           commands=\
                           [
                               ["TSTART","grad_m"],
        ...
        lam_programs["set_grad_m"] = \
          nlam.lam_program("set_grad_m",commands=[])
    ...
    if do_stt:
        ...
        template_eq_grad_m_x_current = \
          "dm_dcurrent_$MAT$(i) <- ($STT_PREFACTOR$) * grad_m_$MAT$(i, j) * current_density(j);"
        ...
        lam_programs["set_dm_dcurrent"] = \
          nlam.lam_program("set_dm_dcurrent",
                           commands=\
                           [
                               ["TSTART","stt"],
                               ["SITE-WISE-IPARAMS", "local_grad_m_x_current",
                                ["v_dm_dcurrent", "v_grad_m",
                                 "v_current_density"],
                                []],
