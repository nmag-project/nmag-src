...
def nmag_lam(timestepper,
    ...
    #BEM needs to know which regions have magnetic materials
    bem_inside_regions = [region_number for (region_number,mats) in
                          enumerate( region_materials) if len(mats)>0]
...
"""
    ...
    lam_bem={"BEM":nlam.lam_bem(name="BEM",
                                ...
                                mwe_name="phi",
                                inside_regions=bem_inside_regions,
    ...
    lam_local={"local_H_total":nlam.lam_local("local_H_total",
               ...
               "local_H_total2":nlam.lam_local("local_H_total2",
                                              aux_args=intensive_params,
                                              field_mwes=mwes_for_H_total,
                                              equation=eq_H_total),
               "local_H_anis2":nlam.lam_local("local_H_anis2",
                                             aux_args=intensive_params,
                                             field_mwes=["H_anis","m"],
                                             equation=eq_H_anis),
               "local_E_anis2":nlam.lam_local("local_E_anis2",
                                             aux_args=intensive_params,
                                             field_mwes=["E_anis","m"],
                                             equation=eq_E_anis),
               "local_dmdt2":nlam.lam_local("local_dmdt2",
                                           aux_args=intensive_params,
                                           field_mwes=mwes_for_dmdt,
                                           equation=eq_rhs_full),
               "local_M_from_m2":nlam.lam_local("local_M_from_m2", # set M from m
                                               aux_args=intensive_params,
                                               field_mwes=["m","M"],
                                               equation=eq_M),
               "local_correct_m_lengths2":nlam.lam_local("local_correct_m_lengths2", 
                                               aux_args=intensive_params,
                                               field_mwes=["m"],
                                               c_code=ccode_correct_m_lengths),
               "local_E_total2":nlam.lam_local("local_E_total2",
                                              aux_args=intensive_params,
                                              field_mwes=mwes_for_E_total,
                                              equation=eq_E),
               }
...
""")
    ...
    ocaml.linalg_machine_set_field(lam, master_fields_and_mwes_by_name["pin"][1], "v_pin")
    if thermal:
        ocaml.linalg_machine_set_field(lam, master_fields_and_mwes_by_name["Temperature"][1], "v_Temperature")
        ocaml.linalg_machine_execute(lam,"init_m_inv_sqrtvol",[],[])
        # XXX NOTE: shouldn't I provide the proper intensive parameters here?!??
    
    # The linalg_machine is set up now.
    ...
    # that drives this linalg_machine is the duty of the simulation object!
