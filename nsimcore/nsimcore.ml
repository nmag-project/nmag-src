...
let version_string = (String.concat ":" ["$Id: pyfem3.ml 6099 2009-03-25 12:07:19Z tf $";
		      (Nlog.version());
		      (Snippets.version());
		      (Qhull.version());
		      (Mt19937.version());
...
(* We had do hack in a type specification here, to satisfy the compiler while we were incrementally exposing pyfem3*)
let pyfem_ccpla: ((Nsim.nsim_ccpla_opcode, Mpi_petsc.matrix Nsim.nsim_ccpla_resource) Ccpla.ccpla) option ref = ref None;;
...
let _py_mpi_hello =
  python_pre_interfaced_function
    ~doc:"Print hello on rank=0 node, and report number of mpi nodes (mpi-testing)"
    [||]
    ( fun args -> 
      let () = (Mpi_petsc.mpi_hello ()) in 
    pynone() 
);;

let _py_ccpla_hello =
  python_pre_interfaced_function
    ~doc:"Ping hello messages from all nodes in the Centrally Coordinated Parallel Linear Algebra (CCPLA) Machine (useful for mpi-testing)"
    [||]
    ( fun args -> 
  ...
  python_pre_interfaced_function
    ...
    (fun args ->
	 ...
	 (fun ba_data ->
	    r_result := 
	      pylist_fromarray
		(Array.init (Array.length sites_and_dofs)
		   (fun nr_site ->
...
let _py_linalg_machine_ccpla_resources =
  python_pre_interfaced_function
...
(*let _py_raw_cvode_advance =
  python_pre_interfaced_function
    ...
    (fun args ->
       ...
       let result_time =
	 Sundials_sp.cvode_advance cvode target_time v_result max_steps
       in
...
*)
let _py_raw_make_linalg_machine	= (* XXX TO BE EXTENDED QUITE DRAMATICALLY IN THE FUTURE! XXX *)
  python_pre_interfaced_function
    [|ListType;	(* mwes *)
    ...
    (fun args ->
	 ...
	 Array.map
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine") in
       let mwes =
	 py_homogeneous_list_as_array
	   "Mesh-With-Elements"
	   ...
	   args.(0)
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine mwes") in
	 ...
	 Array.map
	   ...
	   (pylist_toarray args.(1))
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine ibuffers") in
	 ...
	 Array.map
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine op_matrices") in
	 ...
	 Array.map
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine dense_matrices") in
       let spec_ksps =
	 Array.map
	   (fun spec ->
	      let data = guarded_pylist_toarray spec in
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine ksps") in
       let spec_swexs =
	 Array.map
	   (fun spec ->
	      let data = guarded_pylist_toarray spec in
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine swexs") in
       let spec_jplans =
	 Array.map
	   (fun spec ->
	      let data = guarded_pylist_toarray spec in
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine jplans") in
       let spec_commseqs =
	 Array.map
	   (fun spec ->
	      let data = guarded_pylist_toarray spec in
	 ...
	 (pylist_toarray args.(9))
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine commseqs") in
       let linalg_script =
	 {
	  ...
	  las_timesteppers=spec_timesteppers;
       ...
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine before make_linalg_machine") in
       let machine =
	 make_linalg_machine
	   ?ccpla:(!pyfem_ccpla)
	   ...
	   linalg_script
       in
       let () = logdebug (Printf.sprintf "raw_make_linalg_machine before returning OCamlPill") 
       in
...
let _py_linalg_machine_get_timers =
  python_pre_interfaced_function
    [|CamlpillType;ListType|]
    (fun args ->
       let linalg_machine = linalg_machine_from_ocamlpill args.(0) in
...
let _py_linalg_machine_initialize_timestepper =
  python_pre_interfaced_function
...
let _py_linalg_machine_extract_timestepper_cvode =
  python_pre_interfaced_function
    ...
    (fun args ->
       ...
       (* Removed this line for next release; aim to put it back in afterwards (fangohr 29/05/08)      *)
       (* let () = Printf.printf "Repair me (Obj.magic). Should only use sundials_sp in future\n%!" in *)
													 
	 ocamlpill_from_cvode (Obj.magic cvode))
;;

let _py_linalg_machine_get_timestepper_timings =
  python_pre_interfaced_function
...
let _py_linalg_machine_set_ksp_tolerances =
  python_pre_interfaced_function
...
let _py_linalg_machine_get_ksp_tolerances =
  python_pre_interfaced_function
...
let _py_linalg_machine_bem_field_strength =
  python_pre_interfaced_function
    [|FloatType; (* epsilon *)
...
let _py_linalg_machine_get_set_field_cofield do_get do_field =
  python_pre_interfaced_function
...
let _py_linalg_machine_get_iparam =
  python_pre_interfaced_function
    [|CamlpillType;StringType|]
    (fun args ->
       let linalg_machine = linalg_machine_from_ocamlpill args.(0) in
...
let _py_linalg_machine_set_iparam =
  python_pre_interfaced_function
...
let () =
  ...
  let () =
    Sundials_sp.sundials_init
      (sundials_path Nsimconf.sundials_nvecserial_lib)  
      (sundials_path Nsimconf.sundials_cvode_lib)
  in
...
let _py_test =
  python_interfaced_function [|FloatType|]
    (fun py_args ->
       let x = pyfloat_asdouble py_args.(0) in
         pyfloat_fromdouble (x*.x))
;;


let create_nsimcore_module () = 
  let () = Printf.printf "Remember to re-implement the argv analysis for MPI.\n" in
(*
let mpi_prefix = "-p4" in
let args = Sys.argv in
(* let () = logdebug (Printf.sprintf "pyfem3: sys.args are %s\n%!" (string_array_to_string args)) in *)
...
*)
  ...
  let () = pyfem_ccpla := Some (Ccpla.setup ~petsc_argv:[|"nsim"|] [|"nsim"|] nsim_opcode_interpreter (fun () -> exit 0)) in
  ...
  let _ =
    Mpi_petsc.petsc_init
      [|"foo"; (*"-info"*)|]
      petscrc_file "[help message]"
      true (* Enable petsc signal handler *)
  in

  
  let _py_mod_ocaml = ourpy_initemptymodule "nsimcore" in
  let _py_mod_ocaml_dict = pymodule_getdict _py_mod_ocaml in
	...
	(* PETSc intelligence *)
	 ("petsc_cpu_cycles", _py_petsc_cpu_cycles);
	  ...
	  ("mpi_hello", _py_mpi_hello);
	  ("mpi_ccpla_hello", _py_ccpla_hello);
	...
	*)
	...
	(* -- Mesh and MWE refinement -- *)
	  ...
	  ("lam_get_ccpla_resources",_py_linalg_machine_ccpla_resources);
	...
	(* --- The new linalg_machine --- *)
	  ("raw_make_lam",_py_raw_make_linalg_machine);
	  ("lam_execute",_py_execute_linalg_machine);
	  ("lam_get_timers",_py_linalg_machine_get_timers);
	  ("lam_ts_init", _py_linalg_machine_initialize_timestepper);
	  ("lam_ts_advance",_py_lam_ts_advance);
	  ("lam_get_ts_cvode",_py_linalg_machine_extract_timestepper_cvode);
	  ("lam_get_ts_timings",_py_linalg_machine_get_timestepper_timings);
	  ("lam_set_ksp_tolerances",_py_linalg_machine_set_ksp_tolerances);
	  ("lam_get_ksp_tolerances",_py_linalg_machine_get_ksp_tolerances);
	  ("lam_bem_field_strength",_py_linalg_machine_bem_field_strength);
	  ("lam_get_field",_py_linalg_machine_get_set_field_cofield true true);
	  ("lam_set_field",_py_linalg_machine_get_set_field_cofield false true);
	  ("lam_get_cofield",_py_linalg_machine_get_set_field_cofield true false);
	  ("lam_set_cofield",_py_linalg_machine_get_set_field_cofield false false);
	  ("lam_get_iparam",_py_linalg_machine_get_iparam);
	  ("lam_set_iparam",_py_linalg_machine_set_iparam);
	(* --- --- *)
