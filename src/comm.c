...
{
  fprintf(stderr,"Finalizing Communicator: %08x\n", Comm_val(v));fflush(stderr);
  MPI_Comm_free(&Comm_val(v));
}

static void caml_mpi_dont_finalize_comm(value v)
{
}
...
value caml_mpi_alloc_comm_nonfinalizing(MPI_Comm c)
{
  value res =
    alloc_final(1 + (sizeof(MPI_Comm) + sizeof(value) - 1) / sizeof(value),
                caml_mpi_dont_finalize_comm, 1, 100);
  Comm_val(res) = c;
...
value caml_mpi_comm_dup(value comm) /* T.F. */
{
  MPI_Comm newcomm;
