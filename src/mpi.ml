...
(*                                                                     *)
(*  Adjustments/Modifications 2005 by Dr. Thomas Fischbacher           *)
(*  Adjustments/Modifications 2008 by Hans Fangohr                     *)
(*                                                                     *)
(***********************************************************************)
...
(* Here, we had to change names from init to mpi_init etc., as we have this for petsc as well. *)
external mpi_init : string array -> unit = "caml_mpi_init"
external mpi_finalize : unit -> unit = "caml_mpi_finalize"

(* We register the MPI exception, but we do not init/finalize, as this is up to Petsc!
*)
...
external send_basic_reporting_length:
    'a -> Marshal.extern_flags list -> rank -> tag -> communicator -> int
    = "caml_mpi_send"

let send_basic data flags rank tag comm =
  let _ = send_basic_reporting_length data flags rank tag comm in ()


let send data dest tag comm =
  send_basic data [Marshal.Closures] dest tag comm

let send_reporting_length data dest tag comm =
  send_basic_reporting_length data [Marshal.Closures] dest tag comm
...
(* T.F. extension: receive variant which also tells us about RECV buffer length: *)
let receive_reporting_length source tag comm =
  let (len, actual_source, actual_tag) = probe source tag comm in
    (len,receive_basic len source tag comm)
...
external broadcast_bigarray_nativeint:
    (nativeint, Bigarray.nativeint_elt, Bigarray.c_layout) Bigarray.Array1.t -> rank -> communicator -> unit
    = "caml_mpi_broadcast_bigarray_nativeint"
external broadcast_bigarray_float:
    (float, Bigarray.float64_elt, Bigarray.c_layout) Bigarray.Array1.t -> rank -> communicator -> unit
    = "caml_mpi_broadcast_bigarray_float"
...
let mpi_status () =
  let my_rank = comm_rank comm_world in
  let my_size = comm_size comm_world in
  Printf.printf "MPI-status: There are %d nodes (this is the master, rank=%d)\n%!" my_size my_rank;;
