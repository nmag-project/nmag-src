(*
   (C) 2005-2009 Dr. Thomas Fischbacher

  ocamlc -I ../nsim_grammars -I ../fastfields -I ../diffop_parser -I ../qhull -I ../snippets -I ../mesh -I ../nlog -I ../mpi_petsc -I ../sundials -i nsim_fem.ml 
...
let r_loginfo = ref (fun msg -> Printf.fprintf stderr "INFO: %s\n%!" msg);;
let r_loginfo2 = ref (fun msg -> Printf.fprintf stderr "INFO2: %s\n%!" msg);;
let r_logdebug = ref (fun msg -> Printf.fprintf stderr "DEBUG: %s\n%!" msg);;

let loginfo msg = (!r_loginfo) msg;;
let loginfo2 msg = (!r_loginfo2) msg;;
let logdebug msg = (!r_logdebug) msg;;

(*

This is how logging for nsim_fem can be re-configured. However, this
should NOT happen in nsim_fem.ml (which should not depend on such an
external module), but in the set-up phase of the main module:

let () =
   let logmsg=Nlog.getLogger "nfem.ocaml" in
   begin
   Nsim_fem.r_loginfo := (fun msg -> logmsg Nlog.Info);
   Nsim_fem.r_loginfo2 := (fun msg -> logmsg Nlog.Info2);
   Nsim_fem.r_logdebug := (fun msg -> logmsg Nlog.Debug);
...
type site = int array;;
 (* Something like [|21;21;29|], which is a concrete realization of the above.
    Vertex numbers are ordered lexicographically, with number of occurrences
    giving the weight.
  *)
...
(* Names of degrees of freedom are e.g.:

   ("Temperature",[||])
   ("E",[|0|]) (x-component of E vector)
   ("Jspin",[|0;2|] (z-component of x-spin current)
 *)
...
(* One problem with the dof data structure is that actually, we would
   ...
   field_layouts, i.e. field_layouts where the corresponding data vectors
   can be added entry-wise.
   ...
   the field_layout data structure, only with re-namings in place on the dofs.

   This means that effectively, a dof can only be interpreted in the
   context of field_layout. In particular, it should neither reference
   its name stem, nor the elements it belongs to.
 *)

(* Also, we want to be able to serialize field_layouts. As these will
   be fairly big data structures, we instead serialize the plan how
   they are built.
*)
...
type element_construction_plan =
  ...
  | ECP_renamed	(* We are not specific on the renaming, as derived FLOs are not cached anyway. *)
...
type dof =
	...
	Encoding: use 2 integers for one simplex+nr_element+index spec:
	int for the simplex, 2x short for nr_element and element_index.
      *)
...
let dof_sx_nel_ix_truncate dof =
  dof.dof_sx_nel_ix <-
    Array.sub dof.dof_sx_nel_ix 0
      (2*dof.dof_sx_nel_ix_fillcount)
;;
...
let dof_sx_nel_ix_extend ?(extra=25) dof =
  ...
  let v_new = Array.init len_new
      (fun n -> if n<len_old then v_old.(n) else 0)
  in
...
let dof_name_to_string dof_name =
  ...
  then name
    ...
    let indices_str =
      String.concat ", " (Array.to_list (Array.map string_of_int indices))
    in
    Printf.sprintf "%s[%s]" name indices_str
;;


(* Note that quite a lot of the bookkeeping information in a
   FLO is redundant.  But that is just the purpose: make
   things available without having to do all kinds of
...
type flo_made_by =
  ...
  | FLOMB_flo_sibling of
      (string * string * (string * string) array * string)
	(* This gets a little bit tricky: the last string replaces the
           FLO we derive from: we have to specify this by name here.
	   Within every nsim linalg_machine, these FLOs are uniquely specified
	   by their name - remember, they have to, because this is how we
	   then create field vectors!
	*)
;;

(* XXX NOTE: this is in flux: the flo_sites_simplex_info_pmid data
   ...
   Every field layout will have to have information available on
...
type  field_layout =
    {flo_mesh: mesh;
     flo_made_by: 'num flo_made_by;
     flo_name: string; (* This is for displaying (also, of fields), as well as for creating petsc names. *)
     flo_elements: 'num element array;
     flo_is_primary: bool;
     flo_subfields: dof_name array;
     flo_properties_by_region: (Mesh.simplex_region,string array) Hashtbl.t;
     flo_sites: site array;
     flo_pmids: int array; (* Array parallel to flo_sites, containing Primary master IDs of sites,
			      for periodic boundary condition support. *)
     flo_sites_simplex_info_pmid: hibernatable_sites_simplex_info_pmid;
     (* For all the sites in the FLO, the info part tells us about:
     ...
     *)
     flo_dofs: dof array; (* A vector of all the DOFs *)
     flo_dof_periodic_images: int array array;
     (* which dof also appears where else? length: nr_dofs, non-periodic entries: [||]
     *)
     (* Note that among the nontrivial entries in flo_dof_periodic_images,
	...
	"equalize" fields.
     *)
     flo_dofs_by_site: (site,dof array) Hashtbl.t; (* Note: in there, the DOFs are sorted by DOF id. *)
     flo_nel_by_simplex_index: int array; (* Element number by simplex index *)
     flo_dof_nrs_by_simplex_index: int array array;
     (* Note: the G-matrix related fields should be refs.
	Reason: if we derive flos #2 and #3 from flo #1,
	and #1 initially did not have G initialized, and
	later on we lazily compute G for flo #2, it should
	also become available for #1 and #3 as well.
     *)
     (* Note: the entries below are "ref" and not "mutable" for better
	sharing between primary and derived flos!
     *)
     flo_dof_neighbour_pairs: (int array * int array) array option ref;
     ...
     *)
     (* NOTE: these entries -- flo_dof_funs_g,flo_dof_funs_g_solver,flo_dof_funs_g_entries --
	are relics that have become obsolete.
     *)
     flo_dof_funs_g: Mpi_petsc.matrix option ref;
     flo_dof_funs_g_solver:
       (?output:Mpi_petsc.vector -> input:Mpi_petsc.vector ->
	 unit -> Mpi_petsc.vector) option ref;
     flo_dof_funs_g_entries: (int * int * float) array ref;
     ...
     *)
     flo_dof_funs_volumes: float array option ref;
	...
	to read off the cvode vector distribution): *)
     flo_dofs_and_distribution_by_dvss:
...
let get_flo_sites_simplex_info_pmid flo =
  let (f,r_data) = flo.flo_sites_simplex_info_pmid in
    match !r_data with
      | Some x -> x
      | None ->
	  let data = f () in
...
let slim_flo_sites_simplex_info_pmid flo =
  let (f,r_data) = flo.flo_sites_simplex_info_pmid in
    r_data := None
;;

(* Note: the field/cofield distinction presumably is to go away soon as well: *)
type fem_field = FEM_field of
   field_layout * dvss * Mpi_petsc.vector;;

type fem_cofield = FEM_cofield of
  field_layout * dvss * Mpi_petsc.vector;;	   

let the_dof_name flo dof = (* Note that every dof must belong to some element! *)
  let nel_ix = dof.dof_sx_nel_ix.(1) in
    (* Note: this entry MUST be present, otherwise the dof does not belong to any FLO site! *)
  let ix = nel_ix land 0xffff in
  let nr_el = nel_ix lsr 16 in
  let elem = flo.flo_elements.(nr_el) in
    elem.el_dof_names.(ix)
;;

let the_dof_femfun flo match_sx dof =
  let fillcount = dof.dof_sx_nel_ix_fillcount in
  let rec walk n =
...
let the_dof_machine flo dof =		(* get the parallel node a given DOF is on *)
  let distrib = flo.flo_distribution in
...
let dof_belongs_to flo dof region_logic =
  let properties_by_region = flo.flo_properties_by_region in
  let no_props=[||] in
...
let dvss_constraint flo dof dvss =
  let result =
    match dvss with
      | None -> true
      | Some a ->
	  let (stem,indices) = the_dof_name flo dof in
	  let p = array_position_if (fun (n,_) -> n=stem) a 0 in
	    if p = (-1) then false
	    else let (_,logic) = a.(p) in
	      dof_belongs_to flo dof logic
  in
    (*
      let ddd =
      ...
      (dof_name_to_string (the_dof_name flo dof))
      dof.dof_nr
      (dvss_to_string dvss)
...
let flo_shortvec_info flo dvss =
    try Hashtbl.find flo.flo_dofs_and_distribution_by_dvss dvss
    with
      | Not_found ->
	  let ht_dof_nrs = Hashtbl.create 100 in
	  (* let ddd = Printf.printf "DDD CPU %d flo \"%s\" distrib %s\n%!" (Mpi_petsc.comm_rank (Mpi_petsc.petsc_get_comm_world())) flo.flo_name (int_array_to_string flo.flo_distribution) in *)
	  let distrib = Array.make (Array.length flo.flo_distribution) 0 in
	  let () = 
	    array_foreach_do flo.flo_dofs
	      (fun dof ->
		 if dvss_constraint flo dof dvss 
		 then
		   if Hashtbl.mem ht_dof_nrs dof.dof_nr 
		   then ()
		   else
		     let () = Hashtbl.add ht_dof_nrs dof.dof_nr true in
		     let m = the_dof_machine flo dof in
		       distrib.(m) <- 1+distrib.(m)
		 else ())
	  in
	  let v_short_to_long = hashtbl_keys ~sorter:compare ht_dof_nrs in
	  let v_long_to_short = Array.make (Array.length flo.flo_dofs) (-1) in
	  ...
	  let () = hashtbl_foreach_do flo.flo_dofs_and_distribution_by_dvss 
	    (fun _ ((_,old_stl,_) as old_entry) ->
	       if old_stl = v_short_to_long then canonical_entry := old_entry else ())
	  in
	  let () = Hashtbl.add flo.flo_dofs_and_distribution_by_dvss dvss (!canonical_entry)
	  in
	  let result = !canonical_entry in
	  let ddd = 
	    let (lts,stl,distrib) = result in
	      logdebug (Printf.sprintf "flo_shortvec_info (CPU %d flo: %s dvss: %s): (lts=#[array length=%d],stl=#[array length=%d],distrib=%s"
			  (Mpi_petsc.comm_rank (Mpi_petsc.petsc_get_comm_world()))
			  flo.flo_name
			  (dvss_to_string dvss)
...
let boundary_shortvec_info dof_stem flo boundary_spec = 
  let the_region_logic = 
    parse_or_error
      ...
      boundary_spec
  in
  flo_shortvec_info flo (Some [|(dof_stem,the_region_logic)|])
;;


let flo_shortvec_selections_match flo sel1 sel2 =
  if sel1=sel2 then true
  else
    let (x,_,_) = flo_shortvec_info flo sel1 in
    let (y,_,_) = flo_shortvec_info flo sel2 in
      (x == y) || (x=y)
;;

let ensure_field_is_unrestricted (FEM_field (flo,restr,_)) =
  let (_,stl,_) = flo_shortvec_info flo restr in
    if Array.length stl = Array.length flo.flo_dofs then ()
    else failwith "FEM_field MUST NOT have DOF restrictions on it!"
;;

let ensure_cofield_is_unrestricted (FEM_cofield (flo,restr,_)) =
  let (_,stl,_) = flo_shortvec_info flo restr in
    if Array.length stl = Array.length flo.flo_dofs then ()
...
(* Note: here we assume that the folded-out DOFs are in the proper order as suggested
   ...
   that have been given to DOFs. In particular, if two FLOs are compatible,
   ...
   is somewhat strange, as we may have FLOs that "basically should be
   ...
   matrix for one flo will also make it available for the others!
   )

   -> XXX Design decision: a flo must list its elements in order of occurrence when
   ...
   Another issue: relabeling of FLOs and disk caching. If we have
   computed the G matrix, we would (usually) like to be able to cache
   it. Now how does this relate to having multiple FLOs around, which
   may share G matrices? We certainly want to avoid the destruction of the
   sharing through saving/loading of FLOs.

   Ad hoc idea: introduce an extra field, which may designate a FLO as
   "primary" or "derived/logical". We enforce the additional
   constraint that only primary FLOs can be used in renaming to derive
   logical ones.

   Only primary FLOs are cached. For the derived ones,
...
(* This is actually used internally,
   when we rename all the elements in an FLO
   to generate another FLO.
 *)
...
let flos_are_compatible ?(restr1=None) ?(restr2=None) flo1 flo2 =
  flo1.flo_mesh == flo2.flo_mesh
  && (flo1.flo_nel_by_simplex_index = flo2.flo_nel_by_simplex_index)
  && (array_compare_pointwise elements_are_compatible
	flo1.flo_elements flo2.flo_elements)
  &&
  ((restr1 = restr2) ||
     (let (_,stl1,_) = flo_shortvec_info flo1 restr1 in
      let (_,stl2,_) = flo_shortvec_info flo2 restr2 in
	stl1 = stl2))
;;
...
let flo_to_string flo =
  Printf.sprintf "#<%d-dim field layout '%s', %d vertices, %d simplices, %d DOFs, subfields: %s>"
    flo.flo_mesh.mm_dim
    flo.flo_name
    (Array.length flo.flo_mesh.mm_points)
    (Array.length flo.flo_mesh.mm_simplices)
    (Array.length flo.flo_dofs)
    (String.concat ", " (Array.to_list (Array.map dof_name_to_string flo.flo_subfields)))
;;

let print_flo fmt flo =
  Format.fprintf fmt "%s" (flo_to_string flo)
;;
...
(* NOTE: below we use the guarantee that the dofs in flo_dofs_by_site
   are sorted lexicographically wrt. their subfields and indices.
*)

let flo_subfield_info flo subfield_name =
  let (_,subfield_indices) = 
    try
      array_find (fun (n,_) -> n=subfield_name) flo.flo_subfields
    with
      | Not_found ->
      failwith (Printf.sprintf "flo_subfield_info: unknown subfield '%s' in flo '%s'"
		  subfield_name flo.flo_name)
  in
    ...
    array_mapfilter
	 ...
	 (fun site_ix dofs ->
	    (site_ix,
	     array_filter
	       (fun dof ->
		  let (dof_stem,_) = the_dof_name flo dof in
		    dof_stem = subfield_name)
	       dofs))
	 flo.flo_dofs_by_site
      )
  in
...
let flo_ensure_has_volumes flo =
  if !(flo.flo_dof_funs_volumes) <> None
  then () (* We have all the information we need *)
  else
    let pname s = Printf.sprintf "%s-FLO-%s" s flo.flo_name in
    let nr_dofs = Array.length flo.flo_dofs in
    let v_vols = Array.make nr_dofs 0.0 in
    let () =
      for nr_dof=0 to nr_dofs-1 do
	dof_sx_nel_ix_iter flo.flo_mesh.mm_simplices flo.flo_dofs.(nr_dof)
	  (fun sx nel ix ->
	     let elem = flo.flo_elements.(nel) in
	     let dof_femfun = elem.el_dof_funs.(ix) in
	     let vol_contrib = femfun_integrate_over_simplex dof_femfun sx in
	     let images = flo.flo_dof_periodic_images.(nr_dof) in
	       if Array.length images <=1 then
		 v_vols.(nr_dof) <- v_vols.(nr_dof)+.vol_contrib
	       else
		 Array.iter 
		   (fun ix_img ->
		      (* let () = Printf.printf "PERIODIC-VOL %s: dof %4d += %f\n%!" flo.flo_name ix_img vol_contrib in *)
		      v_vols.(ix_img) <- v_vols.(ix_img)+.vol_contrib) images)
      done
    in
    let () = 
      for nr_dof=0 to nr_dofs-1 do
	flo.flo_dofs.(nr_dof).dof_volume <- v_vols.(nr_dof);
      done
    in
      flo.flo_dof_funs_volumes := Some v_vols
;;
...
let copy_field (FEM_field (flo,r,v)) =
  FEM_field (flo,r,Mpi_petsc.vector_pack (Mpi_petsc.vector_extract v))
;;

let copy_cofield (FEM_cofield (flo,r,v)) =
  FEM_cofield (flo,r,Mpi_petsc.vector_pack (Mpi_petsc.vector_extract v))
;;

let copy_field_into (FEM_field (flo_s,r_s,v_s)) ((FEM_field (flo_d,r_d,v_d)) as result) =
  let () =
    (if flo_s != flo_d || r_s <> r_d then failwith "copy_field_into: incompatible fields!" else ())
  in
  let _ = Mpi_petsc.vector_copy v_s v_d
  in result
;;

let copy_cofield_into (FEM_cofield (flo_s,r_s,v_s)) ((FEM_cofield (flo_d,r_d,v_d)) as result) =
  let () =
    (if flo_s != flo_d || r_s <> r_d then failwith "copy_cofield_into: incompatible cofields!" else ())
  in
  let _ = Mpi_petsc.vector_copy v_s v_d
  in result
;;
...
let make_field ?(name=gensym "Field_") ?(restriction=None) ?constant_value flo =
  let nr_dof = 
    match restriction with
      | None -> Array.length flo.flo_dofs
      | Some restr ->
	  let (_,long_to_short,_) = flo_shortvec_info flo restriction in
  ...
  in
    FEM_field (flo,restriction,vec)
;;

let make_cofield ?(name=gensym "Cofield_") ?(restriction=None) ?constant_value flo =
  let nr_dof = 
    match restriction with
      | None -> Array.length flo.flo_dofs
      | Some restr ->
  ...
  let vec =
    match constant_value with
	  ...
	  Mpi_petsc.vector_pack (Array.make nr_dof c)
  in
    FEM_cofield (flo,restriction,vec)
;;


(* XXX ZZZ This should use petsc's axpby! *)
let field_axpby ((FEM_field (flo_target,rt,v_target)) as target)
    coeff1 (FEM_field (flo1,r1,v_src1))
    coeff2 (FEM_field (flo2,r2,v_src2))
    =
  if flo1 != flo2 || flo_target != flo1 || r1 <> r2 || rt <> r1
  then failwith "field_axpby: incompatible fields provided!"
  else
    let nr_dofs = Array.length flo1.flo_dofs in
    let () =
...
let cofield_to_field ?target ?(box_method=false) ((FEM_cofield (flo,_,v)) as cofield) =
  let () = ensure_cofield_is_unrestricted cofield in
  let gs =
    if box_method 
    then
      let () = flo_ensure_has_volumes flo in
      let v_vols = match !(flo.flo_dof_funs_volumes) with
	| None -> impossible()
	| Some v -> v
      in
      fun ?output ~input () ->
	let v_out = 
	  match output with
	    | None ->
		let v = Mpi_petsc.vector_create (Array.length flo.flo_dofs) (gensym "field-") in
		let () = Mpi_petsc.vector_assemble v in
		  v
	    | Some v -> v
	in
	let () =
	  Mpi_petsc.with_petsc_vector_as_bigarray v_out
	    (fun ba_out ->
		 ...
		 (fun ba_input ->
		    for i=0 to Array.length flo.flo_dofs-1 do
  ...
  in
    match target with
      | None -> FEM_field (flo,None, (gs ~input:v ()))
      | Some (FEM_field (flo_target,_,v_target) as field) ->
	  let () = ensure_field_is_unrestricted field in
	    if flo_target != flo
	    then failwith "cofield_to_field: invalid target field"
	    else
	      FEM_field (flo, None, (gs ~input:v ~output:v_target ()))
;;

let field_to_cofield ?target ?(box_method=false) ((FEM_field (flo,_,v)) as field) =
  let () = ensure_field_is_unrestricted field in
  let () = flo_ensure_has_volumes flo in
  let v_vols = match !(flo.flo_dof_funs_volumes) with
    | None -> impossible()
  ...
  let vc = 
    match target with
      | None ->
	  let v = Mpi_petsc.vector_create (Array.length flo.flo_dofs) (gensym "field-") (* XXX gensym! *)
	  in 
	  let () = Mpi_petsc.vector_assemble v in
	    v
      | Some ((FEM_cofield (flo_target,_,v_target)) as cofield) ->
	  let () = ensure_cofield_is_unrestricted cofield in
	    if flo_target != flo
  ...
  in
    FEM_cofield (flo,None,vc)
;;


let field_flo (FEM_field (flo,_,_)) = flo;;
let cofield_flo (FEM_cofield (flo,_,_)) = flo;;

let field_do_scale (FEM_field (flo,_,v)) x =
  Mpi_petsc.vector_scale v x
;;

let cofield_do_scale (FEM_cofield (flo,_,v)) x =
  Mpi_petsc.vector_scale v x
;;

(* Source "pushes" its entries into dst *)
let field_push (FEM_field (flo_src,restr_src,v_src)) (FEM_field (flo_dst,restr_dst,v_dst)) =
  let () = (if flos_are_compatible flo_src flo_dst then () else failwith "field_push: incompatible FLOs!") in
  let (src_lts, src_stl, _) = flo_shortvec_info flo_src restr_src in
  let (dst_lts, dst_stl, _) = flo_shortvec_info flo_dst restr_dst in
...
let field_pull (FEM_field (flo_src,restr_src,v_src)) (FEM_field (flo_dst,restr_dst,v_dst)) =
  let () = (if flos_are_compatible flo_src flo_dst then () else failwith "field_pull: incompatible FLOs!") in
  let (src_lts, src_stl, _) = flo_shortvec_info flo_src restr_src in
  let (dst_lts, dst_stl, _) = flo_shortvec_info flo_dst restr_dst in
    Mpi_petsc.with_petsc_vector_as_bigarray v_src
      (fun ba_src ->
...
let cofield_push (FEM_cofield (flo_src,restr_src,v_src)) (FEM_cofield (flo_dst,restr_dst,v_dst)) =
  let () = (if flos_are_compatible flo_src flo_dst then () else failwith "cofield_push: incompatible FLOs!") in
  let (src_lts, src_stl, _) = flo_shortvec_info flo_src restr_src in
  let (dst_lts, dst_stl, _) = flo_shortvec_info flo_dst restr_dst in
    Mpi_petsc.with_petsc_vector_as_bigarray v_src
      (fun ba_src ->
...
let cofield_pull (FEM_cofield (flo_src,restr_src,v_src)) (FEM_cofield (flo_dst,restr_dst,v_dst)) =
  let () = (if flos_are_compatible flo_src flo_dst then () else failwith "cofield_pull: incompatible FLOs!") in
  let (src_lts, src_stl, _) = flo_shortvec_info flo_src restr_src in
  let (dst_lts, dst_stl, _) = flo_shortvec_info flo_dst restr_dst in
    Mpi_petsc.with_petsc_vector_as_bigarray v_src
      (fun ba_src ->
...
(* Internal function *)
let write_flo_to_fh ?(handle=stdout) flo = failwith "XXX WRITE ME - write_flo_to_fh!";;

let write_flo filename flo =
  let h = open_out filename in
  let () = write_flo_to_fh ~handle:h flo in
    close_out h
;;

let flo_sibling new_name element_renaming_prefix dof_renamings flo =
  if not flo.flo_is_primary
  then failwith "Can only derive from primary FLOs!"
    ...
    let rename_dof (n,ix) = (rename n,ix) in
    let flo_mb = 
      FLOMB_flo_sibling (new_name,element_renaming_prefix,dof_renamings,flo.flo_name)
    in
      {
	flo_mesh=flo.flo_mesh;
	flo_made_by=flo_mb;
	flo_name=new_name;
	flo_elements=
	  Array.map (_element_rename_dofs element_renaming_prefix dof_renamings) flo.flo_elements;
	flo_is_primary=false;
	flo_subfields=Array.map rename_dof flo.flo_subfields;
	flo_properties_by_region=flo.flo_properties_by_region;
	flo_sites = flo.flo_sites;
	flo_pmids = flo.flo_pmids;
	flo_sites_simplex_info_pmid = flo.flo_sites_simplex_info_pmid;
	flo_dofs = flo.flo_dofs; (* Note that this large chunk of data just can be shared! *)
	flo_dof_periodic_images = flo.flo_dof_periodic_images;
        flo_dofs_by_site = flo.flo_dofs_by_site;
	flo_nel_by_simplex_index = flo.flo_nel_by_simplex_index;
	flo_dof_nrs_by_simplex_index = flo.flo_dof_nrs_by_simplex_index;
	flo_dof_neighbour_pairs = flo.flo_dof_neighbour_pairs;
	flo_dof_funs_g = flo.flo_dof_funs_g;
	flo_dof_funs_g_solver = flo.flo_dof_funs_g_solver;
	flo_dof_funs_g_entries = flo.flo_dof_funs_g_entries;
        flo_dof_funs_volumes = flo.flo_dof_funs_volumes;
	flo_distribution = flo.flo_distribution;
	flo_dofs_and_distribution_by_dvss=
	  flo.flo_dofs_and_distribution_by_dvss;
      }
;;

let flo_memstats flo =
  let mf = Snippets.memory_footprint in
  let stats = [|
    ("flo_mesh", mf (flo.flo_mesh));
    ("flo_made_by", mf (flo.flo_made_by));
    ("flo_name", mf (flo.flo_name));
    ("flo_elements", mf (flo.flo_elements));
    ("flo_is_primary", mf (flo.flo_is_primary));
    ("flo_subfields", mf (flo.flo_subfields));
    ("flo_properties_by_region", mf (flo.flo_properties_by_region));
    ("flo_sites", mf (flo.flo_sites));
    ("flo_pmids", mf (flo.flo_pmids));
    ("flo_sites_simplex_info_pmid", mf (flo.flo_sites_simplex_info_pmid));
    ("flo_dofs", mf (flo.flo_dofs));
    ("flo_dof_periodic_images", mf (flo.flo_dof_periodic_images));
    ("flo_dofs_by_site", mf (flo.flo_dofs_by_site));
    ("flo_nel_by_simplex_index", mf (flo.flo_nel_by_simplex_index));
    ("flo_dof_nrs_by_simplex_index", mf (flo.flo_dof_nrs_by_simplex_index));
    ("flo_dof_neighbour_pairs", mf (flo.flo_dof_neighbour_pairs));
    ("flo_dof_funs_g", mf (flo.flo_dof_funs_g));
    ("flo_dof_funs_g_solver", mf (flo.flo_dof_funs_g_solver));
    ("flo_dof_funs_g_entries", mf (flo.flo_dof_funs_g_entries));
    ("flo_dof_funs_volumes", mf (flo.flo_dof_funs_volumes));
    ("flo_distribution", mf (flo.flo_distribution));
    ("flo_dofs_and_distribution_by_dvss", mf (flo.flo_dofs_and_distribution_by_dvss));
    ("flo (total)", mf flo);|]
  in
    stats
;;

let flo_memstats_str stats =
...
let make_compatible_field (FEM_field (flo,r,v)) =
  FEM_field (flo,r, Mpi_petsc.vector_duplicate v)
;;
...
let field_cofield_inner_product
    (FEM_field (flo_f,r1,data_f))
    (FEM_cofield (flo_cf,r2,data_cf)) =
  if not (flos_are_compatible flo_f flo_cf) || r2 <> r2
  then failwith "Problem: incompatible field and cofield given for field_cofield_inner_product"
  else
    let sum = ref 0.0 in
    let nr_dofs = Array.length flo_f.flo_dofs in
    let () =
...
let flo_dof_jumps flo =
  ...
  let () =
    do_foreach_count flo.flo_elements
      (fun n_elem1 elem1 ->
	 do_foreach_count flo.flo_elements
	   (fun n_elem2 elem2 ->
...
(* Guarantees provided by make_flo:

   (1) flo_elements contains elements in the order of appearance, as we walk through
       all simplices.

   (2) flo_subfields entries are listed in lexicographical order.

   (3) The flo_sites_simplex_info array lists entries in
...
let debug_make_flo 
  ...
  (* Next, we have to make all the DOFs.
     XXX Actually, there is more bookeeping information the FLO should
     know about, such as all the relevant sites in the FLO.
   *)
  ...
  in 
    {
      ...
      flo_made_by=flo_mb;
      flo_name=name;
      flo_elements=v_elements;
      flo_is_primary=true;
      flo_subfields=subfields;
      flo_properties_by_region=ht_prop_by_region;
      flo_sites=v_sites;
      flo_pmids=v_pmid;
      flo_sites_simplex_info_pmid=the_hibernatable_sites_simplex_info_pmid;
      flo_dofs=dofs;
      flo_dof_periodic_images=periodic_images;
      flo_dofs_by_site=ht_dofs_by_site;
      flo_nel_by_simplex_index=v_nel_by_sx_index;
      flo_dof_nrs_by_simplex_index=dof_nrs_by_simplex_index;
      (* -- lazy entries -- *)
      flo_dof_neighbour_pairs = ref None;
      flo_dof_funs_g = ref None;
      flo_dof_funs_g_solver = ref None;
      flo_dof_funs_g_entries = ref [||];
      flo_dof_funs_volumes = ref None;
      flo_distribution=flo_distribution;
      flo_dofs_and_distribution_by_dvss = Hashtbl.create 1;
    }
;;
...
let make_flo name
  ...
  let () =
    if debug then
      let report = flo_memstats_str (flo_memstats flo) in
        Printf.printf
...
(*  in
  let ddd = 
    begin
      Printf.printf "DDD CPU %d === flo \"%s\" properties by region ===\n" (Mpi_petsc.comm_rank (Mpi_petsc.petsc_get_comm_world())) flo.flo_name;
      Hashtbl.iter (fun (Body_Nr k) v -> Printf.printf "%d => %s\n" k (string_array_to_string v)) flo.flo_properties_by_region
    end
*)
  in
    flo
...
let flo_ensure_has_neighbour_pairs flo =
  match !(flo.flo_dof_neighbour_pairs) with
    | Some _ -> () (* We already have that information *)
    | None ->
	let v_ht_pairs =
	  Array.init
	    (Array.length flo.flo_subfields) (fun _ -> Hashtbl.create 1987)
	in
	...
	let () =
	  array_foreach_do_n flo.flo_nel_by_simplex_index
	    (fun nr_simplex nr_el ->
	       let sx = flo.flo_mesh.mm_simplices.(nr_simplex) in
	       let el = flo.flo_elements.(nr_el) in
			...
			let ht = v_ht_pairs.(ix_subfield) in
			  begin
			    for i=0 to nr_sites-1 do
			      ...
			      let dof_i =
				array_find
				  (fun dof -> the_dof_name flo dof = leading_dof) 
				  (Hashtbl.find flo.flo_dofs_by_site site_i)
			      in
				...
				for j=i+1 to nr_sites-1 do
				  ...
				  let dof_j =
				    array_find
				      (fun dof -> the_dof_name flo dof = leading_dof) 
				      (Hashtbl.find flo.flo_dofs_by_site site_j)
				  in
	...
	let neighbours =
	  Array.map 
	    ...
	    v_ht_pairs
	in
	  flo.flo_dof_neighbour_pairs := Some neighbours
;;     
...
let flo_dof_connectivity flo dof_stem =
  let () = flo_ensure_has_neighbour_pairs flo in
  let nr_subfield = array_position_if (fun (n,_) -> n = dof_stem) flo.flo_subfields 0 in
  let () =
    (if nr_subfield == (-1)
     then failwith (Printf.sprintf "flo_dof_connectivity: bad dof name stem: '%s'" dof_stem)
     else ())
  in
  let (_,max_indices) = flo.flo_subfields.(nr_subfield) in
  let neighbours =
    match !(flo.flo_dof_neighbour_pairs) with
      | None -> impossible ()
...
let flo_matnullspace_vectors ~fun_make_vector ~fun_add_entry ~fun_assemble_vector flo dof_stems =
  let pieces = 
    array_join (Array.map (fun s -> flo_dof_connectivity flo s) dof_stems) in
...
let equalize_periodic_field
    (FEM_field (flo_src,restr_src,v_src))
    (FEM_field (flo_dst,restr_dst,v_dst))
    opt_dof_stems =
  let () = (if flo_src != flo_dst	(* Note: testing for EQ-ness here! *)
	      || restr_src <> restr_dst
	    then failwith "equalize_periodic_field: field types must be PRECISELY equal!"
	    else ())
  in
  let flo = flo_src and restr = restr_src in
  ...
  match restr_src with
    ...
    | None ->
	let periodic_info = flo.flo_dof_periodic_images in
	let nr_dofs = Array.length flo.flo_dofs in
	  Mpi_petsc.with_petsc_vector_as_bigarray v_src
	    (fun ba_v_src ->
	       Mpi_petsc.with_petsc_vector_as_bigarray v_dst
		 (fun ba_v_dst ->
			...
			if Array.length p_i > 0 && p_i.(0) = i
			  && (let (stem,_) = the_dof_name flo flo.flo_dofs.(i) in
...
(* The definition of diffop_id had to be moved upwards, so that
   flo_ensure... can use it! *)

let integrate_field_slow ?dof_stem (FEM_field (flo,_,v) as field) =
  let () = ensure_field_is_unrestricted field in
  let () = flo_ensure_has_volumes flo in 
  let vols = match !(flo.flo_dof_funs_volumes) with
    | None -> impossible ()
    | Some x -> x
  in
  ...
  let () =
    Mpi_petsc.with_petsc_vector_as_bigarray v
	 ...
	 for i=0 to nr_dofs-1 do
	   ...
	   let dof_name = the_dof_name flo this_dof in
	     if not(filter_this_dof_name dof_name)
	       || (let a = flo.flo_dof_periodic_images.(i) in Array.length a > 1 && a.(0) <> i)
...
let integrate_field_fast dof_stem (FEM_field (flo,_,v) as field) =
  let () = ensure_field_is_unrestricted field in
  let () = flo_ensure_has_volumes flo in 
  let (Some vols) = !(flo.flo_dof_funs_volumes) in
  let all_dofs = flo.flo_dofs in
  let nr_dofs = Array.length all_dofs in
  try
    let (stem, max_indices) =
      array_find (fun (n, _) -> n = dof_stem) flo.flo_subfields in
    ...
    let () =
      Mpi_petsc.with_petsc_vector_as_bigarray v
	   ...
	   for i=0 to nr_dofs-1 do
	     let this_dof = all_dofs.(i) in
	     let stem, indices = the_dof_name flo this_dof in
	       if stem <> dof_stem
	         || (let a = flo.flo_dof_periodic_images.(i) in
...
let probe_field (FEM_field (flo,_,v_field) as field) ?dof_stem pos =
  let simplices=flo.flo_mesh.mm_simplices in
  let () = ensure_field_is_unrestricted field in
  let filter_this_dof_name =
    match dof_stem with
      | None -> (fun _ -> true)
  ...
  let ht_field = Hashtbl.create 5 in
    try
      let (simplex,coords_L) = mesh_locate_point flo.flo_mesh pos in
	...
	(* ^ Note: this cannot be None! *)
      ...
      let elem = flo.flo_elements.(flo.flo_nel_by_simplex_index.(sx_id))
      and dofs = Array.map (fun n -> flo.flo_dofs.(n)) flo.flo_dof_nrs_by_simplex_index.(sx_id)
      in
      let () = Mpi_petsc.with_petsc_vector_as_bigarray v_field
	(fun ba_field ->
	   Array.iter
	     (fun dof ->
		let dof_name = the_dof_name flo dof in
		  if not(filter_this_dof_name dof_name)
		  then ()
		  else
...
let mesh2d_plot_scalar_field
    ...
    ?(plot_edges=true)
    plot_dof_name
    (FEM_field(flo,_,v_petsc) as field)
    color_scheme
    filename
    =
  let () = ensure_field_is_unrestricted field in
  let data = Mpi_petsc.vector_extract v_petsc in
  let nr_simplices = Array.length flo.flo_mesh.mm_simplices in
  ...
  begin
    ...
    for sx_id = 0 to nr_simplices-1 do
      (* let el = flo.flo_elements.(flo.flo_nel_by_simplex_index.(sx_id)) in *)
      let v_dofs = Array.map (fun n -> flo.flo_dofs.(n)) flo.flo_dof_nrs_by_simplex_index.(sx_id)
      in
      let relevant_dofs =
	array_filter
	  (fun d -> the_dof_name flo d = plot_dof_name)
	  v_dofs in
    ...
    (if plot_edges then
	 ...
	 let relevant_dofs =
	   array_filter
		...
		&& (the_dof_name flo d= plot_dof_name))
	     v_dofs in
	 let nr_relevant = Array.length relevant_dofs in
	   if nr_relevant = 3
	   then
...
let debugprint_field (FEM_field (flo,r,v_petsc) as field) =
  let () = ensure_field_is_unrestricted field in
...
let debugprint_cofield (FEM_cofield (flo,r,v_petsc) as cofield) =
  let () = ensure_cofield_is_unrestricted cofield in
  let v_ml = Mpi_petsc.vector_extract v_petsc in
  Array.iteri
    (fun nr_dof dof ->
...
let sample_field ?(petsc_name=gensym "field") flo f =
  ...
  in
  FEM_field(flo,None,v_target)
;;

(* Using a different interface for the function f...
   XXX should allow target argument!
 *)
let sample_field_type2 ?(petsc_name=gensym "field") flo f =
  let () = logdebug (Printf.sprintf "Inside sample field!%!") in
  let nr_dofs = Array.length flo.flo_dofs in
  let () = logdebug (Printf.sprintf "Creating petsc vector%!") in
  let v_target = Mpi_petsc.vector_create nr_dofs petsc_name in
...
(* mf: I needed this function for hysteresis loops (to change the H field)
   I hope I did not create a redundant funcion...
 *)
let set_field field f =
  let () = ensure_field_is_unrestricted field in
  let FEM_field (flo,_,v_target ) = field in
  let nr_dofs = Array.length flo.flo_dofs in
  let () =
    Mpi_petsc.with_petsc_vector_as_bigarray v_target
      (fun ba_target ->
...
(* We need means to produce a fine mesh from a coarse mesh, as well as
   means to transport fields from corresponding flos - that is,
   essentially a wrapper around make_flo that takes two meshes and
   produces two flos and data vector translation functions in both 
   directions.
...
let make_coarse_fine_flo
  ...
  let flo_coarse = make_flo name_coarse fun_simplex_to_element mesh_coarse
  and flo_fine = make_flo name_fine fun_simplex_to_element mesh_fine
  in
  let simplices_coarse = flo_coarse.flo_mesh.mm_simplices
  and simplices_fine = flo_fine.flo_mesh.mm_simplices
  in
  ...
  let coarse_fine_coeffs =
	...
	(Array.length flo_coarse.flo_dofs)
	(Array.length flo_fine.flo_dofs) 
	petsc_name
  in
  ...
  let () = foreach_do_n flo_fine.flo_dof_nrs_by_simplex_index
    (fun fine_sx_ix fine_dof_nrs ->
       let coarse_sx_ix = fine_coarse_simplex_info.(fine_sx_ix) in
       let coarse_sx = flo_coarse.flo_mesh.mm_simplices.(coarse_sx_ix) in
       let fine_sx = flo_fine.flo_mesh.mm_simplices.(fine_sx_ix) in
       let coarse_sx_mx_x_to_L =
	 match coarse_sx.ms_inv_ext_point_coords with
	   | None -> impossible()
	   | Some x -> x
       in
       let coarse_dof_nrs = flo_coarse.flo_dof_nrs_by_simplex_index.(coarse_sx_ix) in
       let ht_coarse_dofs_by_name = Hashtbl.create 10 in
       let ht_fine_dofs_by_name = Hashtbl.create 10 in
       let () = foreach_do fine_dof_nrs
	 (fun fine_dof_nr -> 
	    let fine_dof = flo_fine.flo_dofs.(fine_dof_nr) in
	    let dof_name = the_dof_name flo_fine fine_dof in
	    let fine_dof_el_ix =
	      let (sx,nel,ix) =
		dof_sx_nel_ix_find simplices_fine fine_dof
		  (fun sx nel ix -> sx.ms_id=fine_sx_ix)
	      in
	      let elem = flo_fine.flo_elements.(nel) in
		(elem,ix)
	    in
	 ...
	 *)
	 ...
	 (fun coarse_dof_nr -> 
	    let coarse_dof = flo_coarse.flo_dofs.(coarse_dof_nr) in
	    let dof_name = the_dof_name flo_coarse coarse_dof in
		...
		dof_sx_nel_ix_find simplices_coarse coarse_dof
	      ...
	      in
	      let elem = flo_coarse.flo_elements.(nel) in
		(elem,ix)
	    in
  ...
  let fun_fine_field_to_coarse_cofield
      ...
      ?target
      (FEM_field (flo_f,_,v_data) as field) =
    let () = ensure_field_is_unrestricted field in
    if flo_f != flo_fine
    then failwith "FLO mismatch!"
    else
      let v_target =
	match target with
	  | None -> 
		...
		Mpi_petsc.vector_create 
	      ...
	      in let () = Mpi_petsc.vector_assemble v in
		   v
	  | Some (FEM_cofield (flo_t,_,v) as cofield) ->
	      let () = ensure_cofield_is_unrestricted cofield in
	      if flo_t != flo_coarse
	      then failwith "coarse field -> fine field: wrong target flo!" 
	      else v
      in
      let () = Mpi_petsc.matrix_times_vector coarse_fine_coeffs v_data v_target in
	FEM_cofield (flo_coarse,None,v_target)
  in
  let fun_coarse_field_to_fine_cofield
      ?(petsc_name="v_fine_from_coarse")
      ?target
      (FEM_field (flo_c,_,v_data) as field) =
    let () = ensure_field_is_unrestricted field in
    if flo_c != flo_coarse
    then failwith "FLO mismatch!"
    else
	...
	FEM_cofield (flo_fine,None,v_target)

  in
    (flo_coarse,flo_fine,fun_fine_field_to_coarse_cofield,fun_coarse_field_to_fine_cofield)
;;
...
(* 
   ...
   Should we rather only require flo arguments and allow the resulting
   function to take any vector of fields, which have signature
   compatible to the flo vector passed to site_wise_execute?
   ...
   Note: non-strict flo checks allow us to use an applicator which was
   built to take a field of flo type X on a field of flo type Y if
   flos X and Y are compatible. The main application is to set up a
...
let site_wise_structure_and_offsets_and_defines
    ...
    aux_arg_names
    =
  let () = loginfo (Printf.sprintf "Assembling site-wise offsets for fields %s" (string_array_to_string (Array.map (fun m -> m.flo_name) (Array.append v_field_flos v_cofield_flos)))) in
  let () = logdebug (Printf.sprintf "site_wise_structure_and_offsets_and_defines #0 nr_field_flos=%d nr_cofield_flos=%d" (Array.length v_field_flos) (Array.length v_cofield_flos)) in
  let empty = [||] in
    ...
    *)
  let v_flos = Array.append v_field_flos v_cofield_flos in
  let nr_flos = Array.length v_flos in
    if nr_flos=0 then ([||],[||],[])
    else
      let flo0 = v_flos.(0) in
      let site_pmid =
	let ht = Hashtbl.create 17 in
	let () =
	  Array.iteri
	    (fun n site -> let pmid = flo0.flo_pmids.(n) in Hashtbl.add ht site pmid)
	    flo0.flo_sites
	in
	  (fun site -> Hashtbl.find ht site)
      in
      let dim = flo0.flo_mesh.mm_dim in
      let all_maxnames_and_field_indices_and_lengths =
	let a =
	  array_join
	    (Array.mapi
	       (fun nr_flo flo ->
		  if flo.flo_mesh != flo0.flo_mesh
		  then failwith "site_wise_execute: incompatible FLOs given!"
		  else
		    Array.map
		      (fun ((dof_name,indices) as dn) ->
			 (dn,nr_flo,Array.fold_left ( * ) 1 indices))
		      flo.flo_subfields)
	       v_flos)
	in
	let () = Array.sort compare a in
	...
	*)
	...
	Array.map
	  (fun site ->
	     ...
	     let () = array_foreach_do v_flos
	       (fun flo ->
		  let dofs =
		    try Hashtbl.find flo.flo_dofs_by_site site with | Not_found -> empty
		  in
		    array_foreach_do dofs
		      (fun dof ->
			 let dof_buffer_offset =
			   Hashtbl.find ht_dof_name_to_buffer_offset
			     (the_dof_name flo dof)
			 in
	...
	then [|v_site_info|]
	  ...
	  let nr_machines = Array.length v_flos.(0).flo_distribution in
	  ...
	  let (slice_lengths,slice_offsets) =
	    ...
	    let () =
	      Array.iteri
		(fun nr_site site ->
		   let m = mesh_vertex_machine flo0.flo_mesh site.(0) in
		     a.(m) <- 1+a.(m))
		all_sites
	    in 
	  ...
	  let flo_offsets = Array.map
	    (fun flo ->
		 ...
		 begin
		   ...
		   logdebug (Printf.sprintf "flo='%s', distribution=%s, offsets=%s" flo.flo_name (int_array_to_string distrib) (int_array_to_string offsets));
		   offsets
		 end)
	    v_flos
	  in
		 ...
		 in
		   Array.map
			...
			let modified_indices = 
			  Array.mapi 
			    (fun n ix ->
			       let nr_flo = Char.code str_nr_field.[n] in
				 ix-flo_offsets.(nr_flo).(nr_machine))
			    indices
			in
...
(* Note: we originally had a "strict flo check" preventing us from applying an
...
let site_wise_execute
    ?(strict_flo_check=true)
    field_flos
    cofield_flos
    aux_arg_names c_code
    =
  let (_,par_site_info,extra_defines) =
    site_wise_structure_and_offsets_and_defines field_flos cofield_flos aux_arg_names
  in
  let swex_nocheck = 
  ...
  in
    if strict_flo_check then
      fun z v_fields v_cofields ->
	let fail () = failwith "Field/FLO incompatibility encountered!" in
	  if   Array.length v_fields <> Array.length field_flos
	    || Array.length v_cofields <> Array.length cofield_flos
	  then fail ()
	  else
	    begin
	      for i=0 to Array.length v_fields-1 do
		let FEM_field(flo,_,_) = v_fields.(i) in
		  if flos_are_compatible flo field_flos.(i) then () else fail()
	      done;
	      for i=0 to Array.length v_cofields-1 do
		let FEM_cofield(flo,_,_) = v_cofields.(i) in
		  if flos_are_compatible flo field_flos.(i) then () else fail()
	      done;
...
let ccode_for_tensor_norms flo =
  let subfields = flo.flo_subfields in
...
let fun_tensor_norms flo =
  ...
  let site_wise_fun =
    let (_,par_site_info,extra_defines) =
      site_wise_structure_and_offsets_and_defines [|flo|] [||] varnames_L_max 
    in
      site_wise_executor par_site_info.(0) extra_defines (ccode_for_tensor_norms flo)
  in
...
let flo_simplex_id_to_elem_dof_nr_to_dof_nr flo =
  let simplices = flo.flo_mesh.mm_simplices in
  let nr_simplices = Array.length simplices in
  let result =
    Array.init nr_simplices
      (fun nr_sx ->
	 let elem = flo.flo_elements.(flo.flo_nel_by_simplex_index.(nr_sx)) in
	   Array.make (Array.length elem.el_dof_names) 0)
  in
    (* fill this table *)
  let () = array_foreach_do flo.flo_dofs 
    (fun dof ->
       dof_sx_nel_ix_iter simplices dof
	 (fun sx nel ix ->
...
*)
let _dof_belongs_to_field flo dof field =
  let (dof_name,dof_indices) = the_dof_name flo dof in
    dof_name = field.Ddiffop_parser.f_name
	 ...
	 | Ddiffop_parser.DLOG_true -> true
	 | dlog -> dof_belongs_to flo dof dlog)
;;
...
(* XXX Note: this ad-hoc helper originally was part of ddiffop_vivified,
   ...
   "fields" array 
*)
let ddiffop_mxdim_index_mapping flo fields =
  let nr_machines = Array.length flo.flo_distribution in
  ...
  let fun_check =
    ...
    then
      ...
      fun dof ->
	-1 <> array_position_if (_dof_belongs_to_field flo dof) fields 0
  in
  let short_to_long =
    array_mapfilter 
      ...
      flo.flo_dofs
  in
  let long_to_short = Array.make (Array.length flo.flo_dofs) (-1) in
  let () =
    Array.iteri
      (fun ix_short dof -> 
	 begin
	   long_to_short.(dof.dof_nr) <- ix_short;
	   let nr_machine = the_dof_machine flo dof in
...
let ddiffop_vivified
  ...
  let () =
    (if mesh != flo_ri.flo_mesh
	 ...
	 (* XXX Note: we might have to use a more appropriate exception,
	    ...
	    Should we also check flo_mid ?
	 *)
     else ())
  in
  let flo_mid = match flo_mid with | None -> flo_ri | Some m -> m in
  (* This hack ensures we do not constantly have to discern between having
     flo_mid or not for the sake of the type system alone... Simplifies coding.
   *)
  let for_ddiffop_dof_combinations f = Array.iter f ddiffop.Ddiffop_parser.diff_contribs in
  let (ixmap_le,mx_size_le) =
    let (a,b,_) =
      ddiffop_mxdim_index_mapping flo_le (let (x,_) = ddiffop.Ddiffop_parser.diff_mxdim in x) in
      (a, Array.length b)
  in
  let (ixmap_ri,mx_size_ri) =
    let (a,b,_) =
      ddiffop_mxdim_index_mapping flo_ri (let (_,x) = ddiffop.Ddiffop_parser.diff_mxdim in x) in
      (a,Array.length b)
  in
  let v_periodicity =
	...
	(fun nr_dof_le dof_le ->
	   if -1=array_position_if (_dof_belongs_to_field flo_le dof_le) p_fields 0
	   then
	     [|nr_dof_le|]
	   else
	     let pi = flo_le.flo_dof_periodic_images.(nr_dof_le) in
  ...
  in
    (* When we create the element/element thunks below,
       ...
       a way to know the flo_dof_nrs of the dofs of
       an element which was instantiated for a particular
       simplex.
    *)
  let flo_le_simplex_id_to_elem_dof_nr_to_dof_nr =
    flo_simplex_id_to_elem_dof_nr_to_dof_nr flo_le
  in
  let flo_ri_simplex_id_to_elem_dof_nr_to_dof_nr =
    flo_simplex_id_to_elem_dof_nr_to_dof_nr flo_ri
  in
  let flo_mid_simplex_id_to_elem_dof_nr_to_dof_nr =
    flo_simplex_id_to_elem_dof_nr_to_dof_nr flo_mid
  in
  let nr_regions =
    1+(Array.fold_left
	 (fun sf sx -> 
  ...
  let () = array_foreach_do simplices
      (fun sx ->
	let nel_le = flo_le.flo_nel_by_simplex_index.(sx.ms_id)
	and nel_ri = flo_ri.flo_nel_by_simplex_index.(sx.ms_id)
	and nel_mid =
	  if have_flo_mid
	  then flo_mid.flo_nel_by_simplex_index.(sx.ms_id)
	  else -1
	in
	...
	then ()
	  ...
	  let elem_le = flo_le.flo_elements.(nel_le)
	  and elem_ri = flo_ri.flo_elements.(nel_ri)
	  and elem_mid = 
	    if not have_flo_mid then empty_element
	    else flo_mid.flo_elements.(nel_mid)
	  in
	  let () =
	    array_foreach_do ddiffop.Ddiffop_parser.diff_contribs
		 ...
		 forall_element_dof_nrs_belonging_to_field elem_le fields_lrm.(0)
		   (fun nr_edof_le ->
			...
			(* Note that flo_field_mid only enters via another
			...
			(* Note that the code below has the effect that if we forget field_mid,
			   ...
			   component.
			*)
			if have_flo_mid && Array.length fields_lrm = 3
			then
			  ...
			  (fun femfun_mid nr_edof_mid ->
			     hashtbl_foreach_do ddiffop_contribs
				  ...
				  let fun_process_simplex ~fun_entry_field_mid wrapped_fun_add_contrib sx =
				    let nr_dof_le = flo_le.flo_dof_nrs_by_simplex_index.(sx.ms_id).(nr_edof_le)
				    and nr_dof_ri = flo_ri.flo_dof_nrs_by_simplex_index.(sx.ms_id).(nr_edof_ri)
				    in
				    let dof_le = flo_le.flo_dofs.(nr_dof_le)
				    and dof_ri = flo_ri.flo_dofs.(nr_dof_ri)
				    in
				    let factor_field_mid = 
				      if have_flo_mid
				      then fun_entry_field_mid (flo_mid.flo_dof_nrs_by_simplex_index.(sx.ms_id).(nr_edof_mid))
				      else 1.0
				    in
				      if not(   dof_belongs_to flo_le dof_le fields_lrm.(0).Ddiffop_parser.f_bspec
					     && dof_belongs_to flo_ri dof_ri fields_lrm.(1).Ddiffop_parser.f_bspec)
				      then ()
				      else
	  ...
	  *)
	  ...
	  then (* speed optimization - unfortunately this duplicates some code: *)
	    ...
	    (fun row col coeff ->
	       ...
	       let dof_ri = flo_ri.flo_dofs.(col) in
	       let v_row = v_periodicity.(row) in
		 (* let ddd = (if Array.length v_row > 1 then Printf.printf "DDD periodic row=%s col=%d\n%!" (int_array_to_string v_row) col else ()) in *)
		 for i=0 to Array.length v_row-1 do
		   let row = v_row.(i) in
		   ...
		   let () =
			 ...
			 if _dof_belongs_to_field flo_ri dof_ri field
			 then
	...
	let () = 
	  let diag_ones = ddiffop.Ddiffop_parser.diff_diag_ones in
	    if Array.length diag_ones = 0 then () else
	      hashtbl_foreach_do flo_le.flo_dofs_by_site
		(fun site dofs_le ->
		   let dofs_ri = try Hashtbl.find flo_ri.flo_dofs_by_site site with | Not_found -> [||] in
		     (array_foreach_do diag_ones
			(fun (field_le,field_ri) ->
			   array_foreach_do dofs_le
			     (fun dof_le ->
				if not(_dof_belongs_to_field flo_le dof_le field_le) then ()
				else 
				  array_foreach_do dofs_ri
				    (fun dof_ri ->
				       if not(_dof_belongs_to_field flo_ri dof_ri field_ri) then ()
				       else 
...
let make_sundials_mirage_dof_mapping ?(petsc_name=gensym "midof") ?subfield_restriction flo =
  ...
  let pdofs = (* remove periodic shadow copies of dofs *)
    array_filter
      (fun dof ->
	 let p_i = flo.flo_dof_periodic_images.(dof.dof_nr) in
	   (Array.length p_i <= 1)
	   ||
	     (let (stem,_) = the_dof_name flo dof in
  ...
  let nr_pdofs = Array.length pdofs in
	...
	for i=0 to nr_pdofs-1 do
	  ...
	  let images = flo.flo_dof_periodic_images.(dof_nr) in
