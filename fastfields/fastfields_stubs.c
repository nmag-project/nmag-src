...
CAMLprim value ocaml_c_fastfield_dlopen(value ml_str)
{
  ...
  /* dl_handle=dlopen(String_val(ml_str),RTLD_NOW); */

  dl_handle=dlopen(String_val(ml_str),RTLD_LAZY);

  if(dl_handle==0){fprintf(stderr,"XXX dlopen() error: %s\n",dlerror());fflush(stderr);}

  /* printf("dlopen('%s') -> %p\n",String_val(ml_str),dl_handle);fflush(stdout); */

  block=alloc_final(2, &finalize_dl_handle,1,100);
  Store_c_field(block,1,dl_handle);
