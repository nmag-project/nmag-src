...
# This paths are reconstructed following indications from main python
...
def find_at_least_one(names, path):
    ...
    '''
    for name in names:
        msg("(Searching %s)" % os.path.join(path, name))
...
def find_files(names, paths, prefixes=[''], suffixes=['']):
    ...
    for path in paths:
        all_found = True
        found_files = []
        msg("Searching %s in %s..." % (names, path))
        for name in names:
            msg("Searching '%s'" % name)
...
configs["mpich-libdir"] = (
  "Path containing the library files for mpich",
  "MPICH2_LIBRARY_PATH",
  "MPICH",
  ["mpich"],
  mpich_lib_path
...
configs["mpich-includedir"] = (
  "Path containing the C-header files for mpich",
  "EXTRA_INCLUDE_PATH",
  "MPICH",
  ["mpi.h"],
  std_inc_paths+['/usr/lib/mpich/include']
)
...
configs["petsc-includedir"] = (
  ...
  std_inc_paths + ["/usr/lib/petscdir/2.3.2/include"]
)
...
for opt, arg in recognized:
    ...
    if not processed_opts.has_key(opt):
        print "Warning: ignored option '%s'" % opt
...
for key in configs:
    ...
    found = None
    if 'libdir' in key:
        msg("Searching library files for the library '%s'" % key)
        ...
        msg("Searching C-header files for the library '%s'" % key)
    ...
    if path != None:
        ...
        msg("Adding variable x_INCFLAGS='%s' and x_LDFLAGS='%s' for x='%s'"
            % (incs, libs, id_name))
        configuration[var] = path
        found_files[var] = found
    else:
        print "Warning: configuration failed for '%s'" % key
...
try:
    ...
    print "WARNING: Cannot import arch. Optimisation flags disabled!"
    print "WARNING: Cannot obtain sizeof(int): assuming sizeof(int) = 4."
    sizeof_int = 4
    CFLAGS_ARCH=""
...
# Write s the same configuration in different languages
