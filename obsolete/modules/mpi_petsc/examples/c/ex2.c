...
/* 
   ...
   NOTE: this example, which came with the petsc documentation, needed
   quite some heavy massaging and bugfixing before it could be run.

   WHAT THE HELL IS THAT? DISTRIBUTING BROKEN EXAMPLES WITH THE CODE???
...
int main(int argc,char **argv)
{
  ...
  PetscInt       time_steps_max = 10;  /* default max timesteps */
  PetscErrorCode ierr;
  PetscInt       steps,mybase, myend;
  PetscReal      ftime;                  /* final time */
  PetscReal      dt;
  PetscReal      time_total_max = 100.0; /* default max total time */
  PetscTruth     flg;
  PetscInt       dim, M, N, P, m, n, p, dof;
  DAPeriodicType datype;
  ...
  */ 
  ...
  ierr = DAGetInfo(appctx.da,&dim,&M,&N,&P,&m,&n,&p,&dof,PETSC_NULL,&datype,PETSC_NULL);CHKERRQ(ierr);
  
  fprintf(stderr,"dim=%d, M=%d, N=%d, P=%d, m=%d, n=%d, p=%d, dof=%d\n",dim, M, N, P, m, n, p, dof);fflush(stderr); 
  ...
  */
  ...
  if (flg) {
    /* ierr = TSSetRHSJacobian(ts,A,A,RHSJacobianFD,&appctx);CHKERRQ(ierr); <- does not work! */
  } else {
