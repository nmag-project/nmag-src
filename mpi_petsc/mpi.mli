...
(*** Initialization / Finalization
     - added to the exported interface by T.F.
     We need this if we also have Petsc and if we may
     initialize via Petsc.init in some situations and via
     Mpi.init in others.
 *)
...
val send_reporting_length: 'a -> rank -> tag -> communicator -> int
       (* like [Mpi.send], but returns buffer length *)
...
val receive_reporting_length: rank -> tag -> communicator -> (int * 'a)
         (* Same as [Mpi.receive], but returns (length,data) *)
...
val comm_dup: communicator -> communicator
        (* [Mpi.comm_dup comm] creates a new communicator for the same group
           of processes as the original one, but with a new comminication
	   context.
	*)
